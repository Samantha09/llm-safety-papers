...
与纯文本攻击相比，视觉越狱具有以下特点：
- **难以检测**: 图像的连续性和高维度使得检测恶意输入变得困难
- **绕过文本过滤器**: 安全机制主要针对文本进行训练，图像作为"外语"可能被忽视
- **攻击空间大**: 图像的像素空间几乎是无限的，难以穷举所有可能的攻击模式
...
- **解耦安全与性能**: 安全机制不直接修改模型参数，避免灾难性遗忘
- **即插即用**: 可以与任何现有的MLLM集成，无需重新训练
- **轻量级**: 使用小型检测器和解毒器，计算开销小
- **可解释性**: 明确识别有害内容并进行修正，过程透明
